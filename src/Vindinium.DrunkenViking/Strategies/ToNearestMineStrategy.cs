...
{
	public class ToNearestMineStrategy : Strategy
	{
		public ToNearestMineStrategy(Map map) : base(map) 
		{
		...
		{
			return
				hero.Mines < this.Map.Mines.Length;
		}
		...
		public override MoveDirection GetMove(State state, Tile location, Hero hero, PlayerType playerToMove)
		{
			...
			{
				if (state.Mines[mine.MineIndex] != playerToMove)
				{
